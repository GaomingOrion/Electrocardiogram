...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=55, input_dim=8):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv1d(input_dim, 64, kernel_size=15, stride=2, padding=7,
                               bias=False)
        ...
        # gru
        # self.gru = nn.GRU(512 * block.expansion, 512, batch_first=True, bidirectional=True)
    ...
    def forward(self, x, extra_info):
        ...
        x = self.avgpool(x)
        x = x.squeeze(-1)
        # x = self.fc(torch.cat([x, extra_info], dim=1))
        x = self.fc(x)
        # gru
        # x = x.transpose(1, 2)
        # x, _ = self.gru(x)
...
if __name__ == '__main__':
    ...
    x = torch.randn(4, 8 + 2, 1024)
    extra_info = torch.randn(4, 2)
    m = resnet101()
    y = m(x, extra_info)
