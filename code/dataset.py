...
class ECGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # y
        if self.mode != 'test':
            target_long = torch.tensor(self.targets[fid][0], dtype=torch.long)
            target_float = torch.tensor(self.targets[fid][1], dtype=torch.float)
            ys = np.zeros(55)
            ys[self.file2idx[id_pair]] = 1
            ys = torch.tensor(ys, dtype=torch.float)
            return x, extra, target_long, target_float, ys
        else:
            return x, extra
